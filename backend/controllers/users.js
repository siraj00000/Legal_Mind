...
module.exports = {
  ...
  googleLogin: async (req, res) => {
    try {
      ...
      if (email_verified) {
        ...
        if (user) {
          const token = jwt.sign({ _id: user._id }, process.env.STILL_ALIVE, {
            expiresIn: "3d",
          });
          return res.json({
            status: "Success",
          ...
          let password = email + process.env.DOING_GOOD;
          ...
          const token = jwt.sign({ _id: newUser._id }, process.env.DOING_GOOD, {
            expiresIn: "3d",
          });
          return res.json({
            status: "Success",
            message: "Welcome " + newUser.name + " to your home page",
            user: newUser,
            token,
          });
        }
  ...
  FacebookLogin: async (req, res) => {
    try {
      ...
      let user = await User.findOne({ email });
      if (user) {
        const token = jwt.sign({ _id: user._id }, process.env.DOING_GOOD, {
          expiresIn: "3d",
        });
        return res.json({
          status: "Success",
